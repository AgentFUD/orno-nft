...
def test_owner_can_flip_state_multiple_times():
    ...
    contract.mint("First", "YES")

    assert contract.tokenStates(0) == True

    contract.flip(0, {'from': owner, 'value': Web3.toWei(0.02, 'ether')})
...
def test_non_owners_can_flip_state_multiple_times():
    ...
    contract.flip(0, {'from': account1, 'value': Web3.toWei(0.02, 'ether')})
    assert contract.tokenStates(0) == False

    contract.flip(0, {'from': account2, 'value': Web3.toWei(0.02, 'ether')})
    assert contract.tokenStates(0) == True
